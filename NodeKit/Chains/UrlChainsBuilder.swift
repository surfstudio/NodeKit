...
open class UrlChainsBuilder<Route: UrlRouteProvider> {
    ...
    open func loadData() -> Node<Void, Data> {
        ...
        let transformator = UrlRequestTrasformatorNode<Json, Data>(next: encoding, method: self.method)
        let encoder = RequstEncoderNode(next: transformator, encoding: self.encoding)
