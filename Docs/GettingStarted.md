...
Напрмиер в `gRPC` мы не управляем адресами напрямую, поэтому здесь механика выбоар эндпоинта будет уже другая. 

И так. Теперь нам нужна модель. А так как у нас 3 запроса. Нам нужно 3 модели. 
...
`Decodable` означает, что модель умеет ТОЛЬКО мапится из `json -> dto -> entity`
...
Видимо у нашего сервера сквозная идентифкация и ID документа уникален для всех пользователей (надо же 🙃)
...
На самом деле обычно аутентификация бывает куда сложнее, но стоит заметить, что это повляет только на один метод - auth. Например если нам придет там какой-нибудь токен, то мы можем переписать его вот так:
