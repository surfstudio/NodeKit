//
//  OffsetAsyncPagerLegacyTests.swift
//  IntegrationTests
//
...
final class OffsetAsyncPagerLegacyTests: XCTestCase {
    func testOffsetIteratorReturnsValue() {
        // given

        let data = ["1", "2", "3", "4", "5"]
        let iterator = OffsetAsyncPagerLegacy<[String]>(dataPrivider: { index, offset in
    ...
    func testOffsetIteratorInCaseIfEmptyDataProviderReturnsErr() {
        ...
        let iterator = OffsetAsyncPagerLegacy<[String]>(dataPrivider: nil, pageSize: 5)

        var recived: PagingErrorLegacy?

        // when

        iterator.next().onError { recived = $0 as? PagingErrorLegacy }

        // then

        XCTAssertEqual(recived!, PagingErrorLegacy.dataProviderNotSet)
    }
}
